...
def network_generator(network_type, node, probability = None, neighbors = None):
    ...
    if network_type==1:
        ...
        G = nx.generators.random_graphs.erdos_renyi_graph(n=int(node),p=probability)
    elif network_type==3:
        G = nx.generators.random_graphs.watts_strogatz_graph(n=int(node),k=int(neighbors), p=probability)
    elif network_type==4:
        G = nx.generators.scale_free_graph(n=int(node))
    else:
        G = nx.generators.random_graphs.erdos_renyi_graph(n=int(node),p=0.5)
